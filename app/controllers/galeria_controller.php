...
if($accion === 'subir' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ruta_fisica    = 'C:/xampp/htdocs/MIS_PROYECTOS/CenitSky/public/uploads/' . $cat['slug'] . '/' . $subcarpeta . '/' . $nombre_archivo;
    $ruta_web       = '/MIS_PROYECTOS/CenitSky/public/uploads/' . $cat['slug'] . '/' . $subcarpeta . '/' . $nombre_archivo;

    if(!is_dir(dirname($ruta_fisica))) {
        mkdir(dirname($ruta_fisica), 0755, true);
    }
...
if($accion === 'editar' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($id > 0) {
        $stmt = $pdo->prepare('UPDATE media SET titulo = ?, descripcion = ? WHERE media_id = ?');
        $stmt->execute([$titulo, $descripcion, $id]);
    }

    header('Location: /MIS_PROYECTOS/CenitSky/admin/pages/galeria.php?ok=editado');
    exit;
}
...
if($accion === 'eliminar' && $id > 0) {
    ...
    if($item) {
        $ruta_fisica = 'C:/xampp/htdocs' . $item['archivo'];
